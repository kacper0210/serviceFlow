...
export default function OrdersList() {
  ...
  /** 🔄 Pobieranie zleceń z backendu z filtrami */
  const fetchOrders = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:4000/api/orders?${params.toString()}`
      );
  ...
  /** 👥 Pobieranie klientów (do edycji zleceń) */
  ...
  /** 🔁 Ładowanie danych przy starcie i zmianie filtrów */
  useEffect(() => {
    fetchOrders();
    fetchClients();
  }, [filter]);
  ...
  /** ✏️ Rozpoczęcie edycji */
  const handleEdit = (order) => {
    setEditingOrder(order.id);
    setEditForm({ ...order });
  };

  /** 🖊️ Zmiana pola formularza */
  ...
  /** 🧠 Zmiana filtrów (status, sort, wyszukiwanie) */
